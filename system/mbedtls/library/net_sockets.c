...
#include MBEDTLS_CONFIG_FILE
    ...
    !defined(__HAIKU__) && !defined(MBEDTLS_WITH_HELIOS) && !defined(PLAT_ECR6600) && !defined (PLAT_aic8800m40)
...
#if defined (MBEDTLS_WITH_HELIOS) && !defined (PLAT_ECR6600) && !defined (PLAT_aic8800m40)
                         ...
                         const char *port, int proto )
{
...
#if defined(PLAT_SONY_ALT1350)
        if( lwip_getaddrinfo( host, NULL, &hints, &addr_list, NULL) != 0 )
#else
    ...
    for( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
        ...
        Helios_DataCall_GetInfo((int32_t)cid, 0, &data_call_info);

		if(cur->ai_family == AF_INET)
		{
			sockaddr_u.addr4.sin_family = cur->ai_family;
			sockaddr_u.addr4.sin_port = 0;
			sockaddr_u.addr4.sin_addr = data_call_info.v4.addr.ip;

			ip4_addr = (struct sockaddr_in *)cur->ai_addr;
			ip4_addr->sin_port = htons((unsigned)port);
		}
		else
		{
			...
			ip6_addr->sin6_port = htons((unsigned)port);
		}

		if( bind( ctx->fd, (struct sockaddr *)&sockaddr_u, sizeof(struct sockaddr) ) != 0)
		{
            close( ctx->fd );
...
#if defined (MBEDTLS_WITH_HELIOS) && !defined (PLAT_aic8800m40)
#if defined(PLAT_SONY_ALT1350)
    if( lwip_getaddrinfo( bind_ip, port, &hints, &addr_list, NULL) != 0 )
#else
    if( getaddrinfo_with_pcid( bind_ip, port, &hints, &addr_list, cid ) != 0 )
#endif
