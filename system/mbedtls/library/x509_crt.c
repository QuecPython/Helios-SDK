...
cleanup:
    ...
    struct dirent *entry;
#else
	struct lfs_info sb = {0};
	struct lfs_info entry = {0};
#endif
    char entry_name[MBEDTLS_X509_MAX_FILE_PATH_LEN];
#if !defined (MBEDTLS_WITH_HELIOS)
    DIR *dir = opendir( path );
#else
	QDIR *dir = ql_opendir( path );
#endif
	...
	#else
		else if( ql_stat( entry_name, &sb ) == -1 )
	#endif
        {
            ret = MBEDTLS_ERR_X509_FILE_IO_ERROR;
            goto cleanup;
...
find_parent:
        ...
        {
		#if defined (MBEDTLS_WITH_HELIOS)
			if( ver_chain->trust_ca_cb_result )
			{
		#endif
